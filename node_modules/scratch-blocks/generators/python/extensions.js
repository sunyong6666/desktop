...
Blockly.Python['robotmove_moveForwardDistance']=function(block){
    ...
    let code
    if(dir=='2'){
        code=`icrobot.motor.move_forward(${speed},duration=-1,distance=${distance}*10)\n`
    }else{
        code=`icrobot.motor.move_backward(${speed},duration=-1,distance=${distance}*10)\n`
    }
...
Blockly.Python['robotmove_moveLeftSpeed']=function(block){
    ...
    if(wheel=='0'){
        if(mode=='cm'){
            code=`icrobot.motor.leftmotor_drive(${speed},duration=-1,distance=${num}*10)\n`
        }else if(mode=='秒'){
            code=`icrobot.motor.leftmotor_drive(${speed},duration=${Math.abs(Number(num))},distance=-1)\n`
        }
    }else{
        if(mode=='cm'){
            code=`icrobot.motor.rightmotor_drive(${speed},duration=-1,distance=${num}*10)\n`
        }else if(mode=='秒'){
